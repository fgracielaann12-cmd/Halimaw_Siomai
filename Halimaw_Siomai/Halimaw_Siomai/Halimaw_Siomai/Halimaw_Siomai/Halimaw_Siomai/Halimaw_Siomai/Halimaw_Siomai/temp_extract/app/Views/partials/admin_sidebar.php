...
<nav id="sidebar">
    ...
    <ul class="nav flex-column px-2 mt-3">
        <?php
        $isDashboard = isActive(['items', '', 'dashboard', 'admin/dashboard']);
        if (strpos(uri_string(), 'items/expiring-soon') === 0 || 
            strpos(uri_string(), 'items/deleted') === 0 || 
            strpos(uri_string(), 'items/logs') === 0 ||
            strpos(uri_string(), 'items/unconsumed') === 0) {
            $isDashboard = '';
        }
        ?>
        <li class="nav-item">
            <a class="nav-link <?= $isDashboard ?>" href="<?= site_url('items') ?>">
        ...
        <li class="nav-item">
            <a class="nav-link <?= isActive(['items/unconsumed']) ?>" href="<?= site_url('items/unconsumed') ?>">
