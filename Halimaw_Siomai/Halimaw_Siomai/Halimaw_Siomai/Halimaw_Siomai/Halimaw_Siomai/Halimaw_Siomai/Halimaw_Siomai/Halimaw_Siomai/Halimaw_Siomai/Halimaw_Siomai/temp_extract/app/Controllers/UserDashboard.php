...
class Userdashboard extends BaseController
{
    public function index()
    {
        ...
        foreach ($items as $item) {
            ...
            if ($status === 'expired' && ($item['auto_delete'] ?? 0) == 1 && !in_array($item['status'], ['manually deleted', 'unconsumed'])) {
...
// ================================================
    ...
    // ================================================
    // 🗑 UNCONSUMED PRODUCTS HISTORY
    // ================================================
    public function unconsumedHistory()
    {
        $this->checkLogin('user');
        $deletedModel = new \App\Models\DeletedItemModel();
        
        // Unconsumed items are those that automatically delete without any sales
        $unconsumedItems = $deletedModel
            ->where('status', 'unconsumed')
            ->orderBy('deleted_at', 'DESC')
            ->findAll();

        return view('user/unconsumed', ['items' => $unconsumedItems]);
    }
