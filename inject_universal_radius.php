...
foreach ($iterator as $file) {
    if ($file->isFile() && $file->getExtension() === 'php') {
        ...
        // Remove old unified radius injection if exists to avoid clutter
        $content = preg_replace('/<!-- UNIFIED \d+PX SYSTEM-WIDE RADIUS OVERRIDE -->[\s\S]*?<\/style>/i', '', $content);
        
        // Find </head> and insert right before it
        if (stripos($content, '</head>') !== false) {
            $content = preg_replace('/(<\/head>)/i', $cssBlock . "\n$1", $content);
...
echo "Successfully injected absolute 12px radius override into $filesUpdated files.\n";
